...
contract StandardCampaignTest is Test {
  ...
  function test_campaignMultipleContributionsAndPayout() {
    ...
    assertEq(campaign.numContributors(), 1);
    var (c1AMount, c1Claimed, c1Created) = campaign.contributions(contributorAddress);
    assertEq(c1AMount, 500);
    assertEq(campaign.amountRaised(), 500);
    assertTrue(campaign.contributeMsgValue.value(3000)());
    assertEq(campaign.numContributors(), 1);
    assertEq(campaign.amountRaised(), 3500);
    var (c2AMount, c2Claimed, c2Created) = campaign.contributions(contributorAddress);
    assertEq(c2AMount, 3500);
    ...
    var (c3AMount, c3Claimed, c3Created) = campaign.contributions(address(contributor2));
    assertEq(c3AMount, 1000);
